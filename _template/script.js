/* ============================================================
   INFLUENCER.NAME — Script
   To create a new site: edit only the CONFIG block below.
   Everything else is driven by these values automatically.
   ============================================================ */

'use strict';

/* ── Per-site CONFIG ── EDIT ONLY THIS BLOCK ──────────────── */
const CONFIG = {
  handle:        'influencer.name',          // folder name / URL slug
  displayHandle: '@INFLUENCER.NAME',         // shown on profile card (uppercase)
  location:      'City, Country',            // shown on location tag
  timezone:      'America/New_York',         // IANA timezone string
  timezoneLabel: 'EST',                      // short label shown after time
  telegramUrl:   'https://go.gramflow.link/influencer.name',
  bioLines: [
    'first rotating bio line.',
    'second rotating bio line.',
    'third rotating bio line.',
    'fourth rotating bio line.',
    'fifth rotating bio line.',
  ],
};
/* ─────────────────────────────────────────────────────────── */
...
(function initParticles() {
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    p.className = 'ember'; /* change class + CSS keyframe to match theme */

    const size  = 4  + Math.random() * 7;
    const left  = Math.random() * 106;
    const delay = Math.random() * 20;
    const dur   = 14 + Math.random() * 18;
    const drift = ((Math.random() - 0.35) * 160).toFixed(0) + 'px';
    const spin  = (100 + Math.random() * 300).toFixed(0) + 'deg';
    const op    = (0.25 + Math.random() * 0.45).toFixed(2);
